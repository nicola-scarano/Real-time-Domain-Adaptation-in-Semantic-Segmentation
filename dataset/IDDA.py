...
class IDDA(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # for k, v in self.toCamVidDict.items():
        #     label_copy[label==k] = v
        #
        # label = label_copy
        label = self.map_to_CamVid_func(label)
    ...
    def map_to_CamVid_func(self, idda_labels):
        
        camvid_labels = np.zeros(idda_labels.shape, dtype=np.uint8)

        for i in range(len(self.idda_labels_info['label2camvid'])):
            mask = idda_labels[:, :, 0] == self.idda_labels_info['label2camvid'][i][0]
            camvid_labels[mask] = self.toCamVidDict[self.idda_labels_info['label2camvid'][i][1]]

        return camvid_labels