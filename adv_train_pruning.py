...
from model.discriminator_dropout import DO_Discriminator
...
from utils import reverse_one_hot, compute_global_accuracy, fast_hist, \
    per_class_iu
from utils import apply_weight_sharing
...
def train(args, model, model_D, optimizer, optimizer_D, CamVid_dataloader_train, CamVid_dataloader_val,
    ...
    for i_iter in range(epoch_start_i, args.num_epochs):
        ...
        p = i_iter * 0.05
        module_tups_D = []
        for module in model_D.modules():
            if isinstance(module, torch.nn.Conv2d):
                module_tups_D.append((module, 'weight'))

        prune.global_unstructured(
            parameters=module_tups_D, 
            pruning_method=prune.L1Unstructured,
            amount=p
        )
        for module, _ in module_tups_D:
            prune.remove(module, 'weight')
...
def main(params):
    ...
    IDDA_dataset = IDDA(IDDA_path, IDDA_label_path, IDDA_json_path, CamVid_csv_path,
                        scale=(args.crop_height, args.crop_width), loss=args.loss)
    ...
    #    4) Standard Discriminator model with Dropout
    # model_D = DO_Discriminator(args.num_classes)
    # if torch.cuda.is_available() and args.use_gpu:
    #     model_D = torch.nn.DataParallel(model_D).cuda()

    # # --- Info about models' number of parameters ---
    ...
    # # Models summary before training
    # print("----- Segmentation model before training -----")
    # summary(model, (3, args.crop_height, args.crop_width))

    # print("----- Discriminator model before training -----")
    # summary(model_D, (args.num_classes, args.crop_height, args.crop_width))
    ...
    # -------------------    Train, (final) Pruning and Validation    -------------------
    ...
    # # --------------- Understanding pruning and where it takes place ---------------
    # print("---PRUNED SEGMENTATION MODEL---\n")
    # sums = [0]
    # elements = [0]
    ...
    # # Models summary before training
    # print("----- Segmentation model after training -----")
    # summary(model, (3, args.crop_height, args.crop_width))

    # print("----- Discriminator model after training -----")
    # summary(model_D, (args.num_classes, args.crop_height, args.crop_width))
...
if __name__ == '__main__':
    params = [
        ...
        '--cuda', '0',
        '--batch_size', '4',
        '--save_model_path', './checkpoints_adv_incr_pruning',
        '--context_path', 'resnet101',  # resnet18 or resnet101
        '--optimizer', 'sgd',
        '--optimizer_D', 'adam',
        '--checkpoint_step', '1',
        '--validation_step', '5',
        '--pretrained_model_path', './checkpoints_adv_incr_pruning/latest_dice_loss.pth'
    ]
    main(params)