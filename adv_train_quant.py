...
from model.quantization import Discriminator
...
def train(args, model, model_D, optimizer, optimizer_D, CamVid_dataloader_train, CamVid_dataloader_val, IDDA_dataloader):
    ...
    for i_iter in range(epoch_start_i, args.num_epochs):
        ...
        #quantization
        model_D.qconfig = torch.quantization.get_default_qat_qconfig('fbgemm')
        model_D_fused = torch.quantization.fuse_modules(model_D, [['conv1', 'conv2', 'conv3', 'conv4', 'classifier']])
        model_D_prepared = torch.quantization.prepare_qat(model_D_fused)
        model_D = model_D_prepared
    ...
    model_D_quant = torch.quantization.convert(model_D)
...
if __name__ == '__main__':
    params = [
        ...
        '--cuda', '0',
        '--batch_size', '4',
        '--save_model_path', './checkpoint_adv_quant',
        '--context_path', 'resnet101',  # resnet18 or resnet101
        '--optimizer', 'sgd',
        '--optimizer_D', 'adam',
        '--checkpoint_step', '1',
        '--pretrained_model_path', './checkpoint_adv_quant/latest_dice_loss.pth'
    ]
    main(params)
